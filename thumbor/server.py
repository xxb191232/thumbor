...
def main(arguments=None):  # NOQA
    ...
    if config.USE_GIFSICLE_ENGINE:
        ...
        if server_parameters.gifsicle_path is None:
            raise RuntimeError('If using USE_GIFSICLE_ENGINE configuration to True, the `gifsicle` binary must be in the PATH '
    ...
    context.run_plugins('before-application-start')

    application = importer.import_class(server_parameters.app_class)(context)

    context.run_plugins('after-application-start', application=application)
